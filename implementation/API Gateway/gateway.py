...
# Proxy routes for Microservice 1
@app.api_route("/appointment/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
...
@app.api_route("/appointmenthistory/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
...
@app.api_route("/patient/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
...
# Proxy routes for Microservice 2
@app.api_route("/doctor/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
...
@app.api_route("/operation/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
...
@app.api_route("/appointmenthistory/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
async def appointment_history_proxy(path: str, request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    # Construct the URL for forwarding the request
    forwarded_url = f"{ANALYTICS_SERVICE_URL}/AppointmentHistory/{path}"
